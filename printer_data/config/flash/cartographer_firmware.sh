...
printf "${BLUE}
 ...
 | |___  | (_| | | |    | |_  | (_) | | (_| | | |    | (_| | | |_) | | | | | |  __/ | |   
  \____|  \__,_| |_|     \__|  \___/   \__, | |_|     \__,_| | .__/  |_| |_|  \___| |_|   
                                       |___/                 |_|                          
${NC}"
printf "${RED}Firmware Script ${NC} v1.0.5\n"
...
initialChecks(){
	...
	if [ -d ~/katapult ]; then
		...
		if [[ $canCheck != "" ]]; then
			...
			canbootCheck=$(~/klippy-env/bin/python ~/klipper/scripts/canbus_query.py can0 | grep -v "Application: Klipper" | grep -m 1 "CanBoot")
			...
			katapultCheck=$(~/klippy-env/bin/python ~/klipper/scripts/canbus_query.py can0 | grep -v "Application: Klipper" | grep -m 1 "Katapult")
